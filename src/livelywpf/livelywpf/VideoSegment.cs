...
namespace YetAnotherLosslessCutter
{
    sealed class VideoSegment : Segment
    {
        ...
        public VideoSegment(VideoCutterView window) => host = window;
        [JsonIgnore]
        public DelegateCommand DeleteThisSegment => new DelegateCommand(() =>
        {
            //You really shouldn't do this
            if (Status == ProgressStatus.Idle)
                ((VideoCutterViewModel)host.DataContext).DeleteSegment(this);
            else
                ((VideoCutterViewModel)host.DataContext).DeleteSegmentFromQueue(this);
